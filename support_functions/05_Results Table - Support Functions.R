...
# x = "no_resources"
# in_questionnaire = in_questionnaire
# data = data
# weights_vector_name = weights_vector_name
# main_col_name = "All"
...
analyzer <- function(x, in_questionnaire, data, weights_vector_name,
  ...
  if (class(x_data) %in% c("logical", "numeric", "integer")) {
    if (in_questionnaire) {
      ...
      avg <- 100 * round(wtd.mean(x_data, weights = weights), 4)
    } else {
      avg <- round(wtd.mean(x_data, weights = weights),3)
    }
...
# data = hh_level_data_subset_with_lsci_calcs
...
generate_results_table <- function(data,
  ...
  generic_columns_to_exclude =   c("start", "end", "today","X_submission_time","X_version","X_duration",
                                   "region","hc_idp","type_settlement","camp_code","ki_consent","settlement_origin"
  ...
  unique_or_equal_columns_to_exclude = map2_chr(data,
                                                names(data),{
                                                  # this removes columns that are all unique or all the same?? I don't understand the purpose of this in general
                                                  # ~  ifelse(length(unique(.x)) %in% c(length(.x),1),.y,as.character(NA))
                                                  # removing the 1 allows all equal columns to pass, but not unique
                                                  ~  ifelse(length(unique(.x)) %in% c(length(.x),0),.y,as.character(NA))
                                                }) %>% .[!is.na(.)]
  ...
  if (use_labels == F) {
    ...
    for (i in 1:nrow(results_table_output)) {
      for (j in 2:ncol(results_table_output)) {
        if (results_table_output[i,1]!="Average" & !is.na(results_table_output[i,j])) {
          results_table_output[i,j] = as.character(as.numeric(results_table_output[i,j]))
        }
      }
    }
  }
    ...
    results_table_output <- qpcR:::cbind.na(
    ...
    results_table_output[results_table_output==""] <- NA
    results_table_output[results_table_output == "NaN"] <- NA
...
export_table <- function(res,path) {
  ...
  openxlsx::saveWorkbook(wb, paste0(path,"results_table_",format(Sys.time(), "%a_%b_%d_%Y_%H%M%S"),".xlsx"), overwrite = TRUE)
